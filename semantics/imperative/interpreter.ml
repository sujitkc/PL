...
let rec string_of_stmt = function
    | Expression.Program.Assignment(s, e) -> s ^ " := " ^ (string_of_expr e) ^ ";"
    | Expression.Program.Return(e) -> "return " ^ (string_of_expr e) ^ ";"
    | Expression.Program.Block(slist) -> "{\n" ^ (List.fold_left string_of_stmt slist "") ^ "}"
...
let typecheck_declarations decs =
  let rec iter decs env =
    match decs with
      [] -> env
    | (vname, typ) :: tl -> iter tl (Env.addBinding vname typ env)
  in
...
let rec typecheck_expr e env fenv : Expression.typ =
  ...
  | Expression.Sub(e1, e2)        ->
    (
      match ((typecheck_expr e1 env fenv), (typecheck_expr e2 env fenv)) with
...
let typecheck_fundef (fd : Expression.Program.fundef) env fenv :
...
let typecheck_fundefs (fdefs : Expression.Program.fundef list)
  ...
  let rec iter fdefs env fenv =
    ...
    | fdef :: tl ->
        ...
        iter tl env (Env.addBinding fdef.Expression.Program.fname fdef_typ fenv)
